...
class TemplateBasedWorker extends Worker {
    ...
    render(context, rule) {
        ...
        ejs.renderFile(this._template(rule.template), this.makeWorkerContext(context, rule), function(err, data) {
            result = data ? data : '';
        });
        return result;
    }
    ...
    writeFromTemplate(name, destination) {
        let templateContext = this.makeWorkerContext(this.context, this.rule);
