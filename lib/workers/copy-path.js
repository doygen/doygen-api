...
/* Classes */
const CopyFileWorker = require('./copy-file').Worker;
...
class CopyPathWorker extends CopyFileWorker {
    ...
    doWork() {
        this.files.forEach(file => {
            let output = this.extract(this.data.output, this.makeWorkerContext(this.context, this.rule)) + file.substr(file.lastIndexOf('/'));
...
module.exports.Worker = CopyPathWorker;
