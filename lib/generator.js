...
class Generator {
    ...
    build(input) {
        Builder.run(this.generatorPath, input);
    }
    ...
    context() {
        ...
        context.params = this.getParams(context);
     ...
     */
    addMetadata(context) {
        let metadataJSon = fse.readJsonSync(path.resolve(this._getBaseDir(Level.LOCAL), 'meta', this.input.type, 'metadata.json'), { throws: false });
